...
def update_drops(ai_settings, screen, drops):
    """检查是否有雨滴从屏幕消失，并更新雨滴位置"""
    check_rain_disappear(ai_settings, screen, drops)
    drops.update()
...
def continue_rain(ai_settings, drops):
    """连绵细雨"""
    for drop in drops.sprites():
        drop.rect.y += ai_settings.drop_speed_factor


def check_rain_disappear(ai_settings, screen, drops):
    """有雨滴从屏幕消失时采取相应的措施"""
    for drop in drops.sprites():
        if drop.check_disappear():
            new_drop = Drop(ai_settings, screen)
            new_drop.rect.x = drop.rect.x
            drops.remove(drop)
            drops.add(new_drop)
